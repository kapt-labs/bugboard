...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('-t', action='store_true', help='Update all tasks (get admin urls, last update infos...).')
        parser.add_argument('-c', action='store_true', help='Update comments list for each stored task.')
    ...
    def handle(self, *args, **options):
        ...
        if not self.api_key:
            raise EnvironmentError('You don\'t have any api key set in your environment variable.\nPlease add "BUGHERD_API=yourapikey" to your .env file.')
    ...
    def update_db(self):
        ...
        self.stdout.write(self.style.SUCCESS('   ➡ ' + str(added_users) + ' new ' + self.pluralize(added_users, 'member')))
        self.stdout.write(self.style.SUCCESS('   ➡ ' + str(added_projects) + ' new ' + self.pluralize(added_projects, 'project')))
        self.stdout.write(self.style.SUCCESS('   ➡ ' + str(added_tasks) + ' new ' + self.pluralize(added_tasks, 'task')))
        self.stdout.write(self.style.SUCCESS('   ➡ ' + str(added_tags) + ' new ' + self.pluralize(added_tags, 'tag')))
    ...
    def update_user(self, user):
        ...
        if user['avatar_url'] == "https://www.bugherd.com/images/sidebar/avatar-generic.svg":
    ...
    def update_projects(self):
        ...
        # thx http://sametmax.com/alternative-au-do-while-en-python/ for the concept
        while "There is more content to download.":
            data = self.get_data(self.api_base + 'projects/active.json?page=' + str(page))
    ...
    def update_tasks(self):
        ...
        while "There is more projects to explore.":
            ...
            while "There is more pages to download.":
                data = self.get_data(self.api_base + 'projects/' + str(id_project) + '/tasks.json?page=' + str(page))
    ...
    def update_task_details(self, id_project, t):
        ...
        try:
            ...
            if pytz.UTC.fromutc((datetime.datetime.strptime(t['updated_at'], '%Y-%m-%dT%H:%M:%S.%fZ'))) > task.updated_at:
        ...
        # or create it if it doesn't exist
            ...
            task = Task(
                ...
                created_at=pytz.UTC.fromutc((datetime.datetime.strptime(t['created_at'], '%Y-%m-%dT%H:%M:%S.%fZ'))),
                updated_at=pytz.UTC.fromutc((datetime.datetime.strptime(t['updated_at'], '%Y-%m-%dT%H:%M:%S.%fZ'))),
    ...
    def update_tags(self):
        ...
        while "There is more tags to update.":
            ...
            for t in tag_list[1]:
                ...
                Task.objects.get(id_task=tag_list[0]).tag.add(
                    tag
                )
                self.display_new_action()
    ...
    def update_local_tasks(self):
        ...
        for task in tasks:
            ...
            updated_task = self.get_data(self.api_base + 'projects/' + str(task.project.id_project) + '/tasks/' + str(task.id_task) + '.json')
    ...
    def update_local_comments(self):
        ...
        for task in tasks:
            ...
            comments = self.get_data(self.api_base + 'projects/' + str(task.project.id_project) + '/tasks/' + str(task.id_task) + '/comments.json')
